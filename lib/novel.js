...
class NarouNovelReviews {
  ...
  async scrape(page) {
    ...
    for await (const reviewHandle of reviewHandles) {
      const review = new NarouNovelReview();
...
class NarouNovel {
  ...
  async parseReviewsItem(reviewHandle) {
    try {
      const titleHandle = await reviewHandle.$('.novelinfo a');
      if (titleHandle) {
        this.url = await titleHandle.evaluate(node => node.getAttribute('href'));
        this.title = await titleHandle.evaluate(node => node.innerText);
      }
  ...
  async scrape(page) {
    ...
    try {
      ...
      if (infoHandle) {
        this.status = await infoHandle.evaluate(node => node.innerText);
